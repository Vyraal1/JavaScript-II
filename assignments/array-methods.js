...
let firstNamesAllCaps = fullNames.map(name => name.toUpperCase());
...
console.log("Please notify the following runnners.");
...
console.log("The donation total is " + ticketPriceTotal);

// ==== Challenge 5: Be Creative ====
// Now that you have used .forEach(), .map(), .filter(), and .reduce().  I want you to think of potential problems you could solve given the data set and the 5k fun run theme.  Try to create and then solve 3 unique problems using one or many of the array methods listed above.
// Problem 1
// Make a table that shows how many runners from each company showed up
let runnerCompanyHash = {};
runners.forEach(runner => {
  runnerCompanyHash[runner.company_name]
    ? runnerCompanyHash[runner.company_name]++
    : (runnerCompanyHash[runner.company_name] = 1);
});

console.log(Object.keys(runnerCompanyHash).length);
console.table(runnerCompanyHash);

// Problem 2
// The organizers are thinking about converting into a for-profit event but they are worried about the cost of materials of larger shirts. Make a table that tracks the total donations of each shirt size.
let sizeToDonationTotals = {};
runners.forEach(runner => {
  sizeToDonationTotals[runner.shirt_size]
    ? sizeToDonationTotals[runner.shirt_size] + runner.donation
    : (sizeToDonationTotals[runner.shirt_size] = runner.donation);
});
//how would I add a 3rd value to track such as how many shirts of each size were given?
//and eventually track how many

console.table(sizeToDonationTotals);
// Problem 3 (taken from Chris Bailey)
// Fun run would like to send out special t-shirts for the representatives that donated more than 200 dollars. They need a list of names, e-mail address, the company they represented, and shirt size. Only one representative from each company can receive this shirt so ensure that no duplicate companies exist. Additionally, the agent in charge of e-mailing each representative would like the e-mail address first, and then first and last name concatenated as a properly capitalized string followed by company and shirt size.
let ballers = runners.filter(runner => runner.donation > 200);
let prettyBallers = ballers.map(runner => {
  return `${runner.email} - ${runner.first_name} ${runner.last_name} - ${
    runner.company_name
  } - ${runner.shirt_size}`;
});
console.log(prettyBallers);
